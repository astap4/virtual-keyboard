class Keyboard {
  ...
  addKeys() {
    ...
    for (let i = 0; i < this.#keyboardLayout.keys.length; i += 1) {
      const row = document.createElement('div');
      row.classList.add('row');
      for (let j = 0; j < this.#keyboardLayout.keys[i].length; j += 1) {
  ...
  initListeners() {
    ...
    document.addEventListener('click', (e) => {
      ...
      } else if (key.classList.contains('press')) {
        key.classList.remove('press');
      } else {
        key.classList.add('press');
      }
  ...
  printKey(pressedKey) {
    ...
    screen.focus();
    const letter = pressedKey.querySelector(`.${this.#keyboardLang}`).querySelector(`.${this.#keyboardCase}`).textContent;
